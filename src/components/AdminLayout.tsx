...
export default function AdminLayout() {
  ...
  const menuItems = [
    ...
    { name: 'Add Doctor', path: '/admin/add-doctor', icon: <PlusCircle size={20} /> },
    { name: 'Doctors List', path: '/admin/doctors-list', icon: <Users size={20} /> },
  ];
      ...
      <div className="min-h-screen flex items-center justify-center bg-accent/30 p-4">
        <div className="bg-card p-10 rounded-3xl shadow-2xl shadow-black/5 w-full max-w-md border border-border/50">
          <div className="flex items-center gap-2 mb-8 justify-center">
            ...
            <h1 className="text-3xl font-bold tracking-tight text-foreground">Medicare <span className="text-primary text-sm align-top">Admin</span></h1>
          </div>
          <form onSubmit={(e) => { e.preventDefault(); setAToken('mock_admin_token'); }}>
             <div className="mb-4">
               <label className="block text-sm font-medium mb-1.5 text-foreground">Email</label>
               <input type="email" required defaultValue="admin@medicare.com" className="w-full p-3.5 px-4 rounded-xl border border-border bg-background focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all outline-none" />
             </div>
             <div className="mb-6">
    ...
    <div className="min-h-screen flex flex-col bg-[#F8F9FD]">
      ...
      <div className="flex justify-between items-center px-6 sm:px-10 py-4 bg-card border-b border-border sticky top-0 z-50">
        <div className="flex items-center gap-4">
           ...
           <span className="bg-muted text-foreground border border-border/80 text-xs font-semibold px-3 py-1 rounded-full tracking-wide">ADMIN</span>
        </div>
